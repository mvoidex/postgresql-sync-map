...
insert ms name m = withModel ms name $ \mdl -> Sync.insert (modelSync mdl) (modelTo mdl m)

-- | Select row by condition
select :: Models -> String -> Condition -> TIO SyncMap
select ms name c = withModel ms name $ \mdl -> fmap (modelFrom mdl) (Sync.select (modelSync mdl) (c `mappend` modelCondition mdl))

-- | Exists rows with condition
exists :: Models -> String -> Condition -> TIO Bool
exists ms name c = withModel ms name $ \mdl -> Sync.exists (modelSync mdl) (c `mappend` modelCondition mdl)

-- | Update by condition with values, stored in map
update :: Models -> String -> Condition -> SyncMap -> TIO ()
update ms name c m = withModel ms name $ \mdl -> Sync.update (modelSync mdl) (c `mappend` modelCondition mdl) (modelTo mdl m)

-- | Insert if not exists, update otherwise
-- Returns True on update
insertUpdate :: Models -> String -> Condition -> SyncMap -> TIO Bool
insertUpdate ms name c m = withModel ms name $ \mdl -> Sync.insertUpdate (modelSync mdl) (c `mappend` modelCondition mdl) (modelTo mdl m)
